...
You will need ~unzip~ for ~dap-gdb-lldb-setup~. It downloads a VS-Code extension and
it needs to extract compressed downloaded file.
...
2. Call ~realgud:<debugger-name>~ to start your favorite debugger (for example,
   you can use ~M-x realgud:pdb~ to launch the PDB Python debugger).

** dap-mode with ~+lsp~
1. Call ~dap-gdb-lldb-setup~ function.
2. Use ~dap-debug-edit-template~ to configure debugging settings for your project.
   [[github:WebFreak001/code-debug/blob/master/package.json#L72][Available settings can be inspected here.]] Call ~eval-buffer~ to save your
